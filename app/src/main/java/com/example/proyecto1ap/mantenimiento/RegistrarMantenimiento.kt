...
class RegistrarMantenimiento(application: Application) : AndroidViewModel(application) {
    ...
    private fun cargarVehiculos() {
        viewModelScope.launch {
            repo.vehiculosActivos()
                ...
                .onFailure { e -> _state.update { it.copy(mensaje = "Error cargando vehículos: ${e.message}") } }
        }
    }
    ...
    fun onCosto(v: String) = _state.update { it.copy(costo = v.filter { it.isDigit() || it == '.' }) }
    ...
    fun guardar() {
        ...
        viewModelScope.launch {
            ...
            if (uid == null) {
                _state.update { it.copy(guardando = false, mensaje = "No se pudo identificar al mecánico") }
                return@launch
            }
            ...
            if (resultado.isSuccess) {
                ...
                _state.update {
                    it.copy(
                        guardando = false,
                        exito = true,
                        mensaje = if (fotosOk) "Mantenimiento registrado"
    ...
    private suspend fun subirFotos(mantenimientoId: Long, fotos: List<Uri>): Boolean {
        ...
        fotos.forEachIndexed { i, uri ->
            ...
            val subio = repo.subirFoto(mantenimientoId, bytes, "foto_${i + 1}.jpg")
                .onSuccess { ruta -> repo.guardarEvidencia(mantenimientoId, ruta) }
                .isSuccess
            if (!subio) ok = false
        }
        return ok
